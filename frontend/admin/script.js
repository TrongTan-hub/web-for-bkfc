...
/* -------------------------------------------------------
   CẤU HÌNH
------------------------------------------------------- */
const API_BASE = 'http://localhost:8000'; // Thay khi deploy
...
async function fetchSystemStatus() {
  try {
    ...
    setStatusVal('sys-vectordb', data.vectorstore_ready ? 'Sẵn sàng' : 'Chưa có dữ liệu',
                 data.vectorstore_ready ? 'ok' : 'pending');
    setStatusVal('sys-ragchain', data.rag_chain_ready   ? 'Sẵn sàng' : 'Chưa khởi tạo',
                 data.rag_chain_ready ? 'ok' : 'pending');
    setStatusVal('sys-pdfs', `${data.pdf_count} file`, data.pdf_count > 0 ? 'ok' : 'pending');
    ...
    addLog('Không kết nối được backend. Kiểm tra server đang chạy chưa.', 'error');
  }
}
...
async function authenticate() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/admin/files?x_api_key=${encodeURIComponent(key)}`);
...
async function uploadFiles() {
  if (!isAuthed) { alert('Vui lòng xác thực admin trước!'); return; }
  if (selectedFiles.length === 0) { alert('Chưa chọn file nào!'); return; }
  ...
  try {
    ...
    // Log kết quả
    data.uploaded.forEach(f => addLog(`✅ Upload thành công: ${f}`, 'success'));
    ...
    setTimeout(() => { progressWrap.classList.add('hidden'); progressFill.style.width = '0%'; }, 3000);
...
async function rebuildDB() {
  if (!isAuthed) { alert('Vui lòng xác thực admin trước!'); return; }
  if (!confirm('Rebuild toàn bộ Vector DB? Quá trình này có thể mất vài phút.')) return;

  addLog('🔨 Bắt đầu rebuild Vector DB...', 'warning');

  try {
    const res  = await fetch(`${API_BASE}/api/admin/rebuild?x_api_key=${apiKey}`, { method: 'POST' });
    const data = await res.json();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  dropZone.addEventListener('drop', e => {
    ...
    if (files.length === 0) { alert('Chỉ chấp nhận file PDF!'); return; }
